...
% header: en tête, il y a toutes les infos au cas ou (calibrations,
% filtres, frequences échatillonages....)

% data: données. Les lignes correspondent aux différentes voies, et les
% colonnes à l'échantillonage temporel
...
% File version
       ...
       elseif header.Version == 3
           ...
           header.FileType = fread(fId,1,'uint32');                                
% FileType (0 = RAW data)
           header.EMGworksVersion = char(fread(fId,11,'char'))';                   
% EMGworks Version ("1.0.0.0")
           header.DADLLversion = char(fread(fId,11,'char'))';                      
% DADLL Version ("1.0.0.0")
           header.CardManufacturer = char(fread(fId,255,'char'))';                 
% A/D card Manufacturer
           header.ADCardID = fread(fId,1,'uint32');                                
% A/D card Identifier
           header.ESeries = fread(fId,1,'uint32');                                 
% National Instruments E-Series Card? (0 = NO, 1 = YES)
           header.Channels = fread(fId,1,'uint32');                                
% Number of channels
           header.Samples = fread(fId,1,'uint32');                                 
% Actual number of samples
           header.SamplesDesired = fread(fId,1,'uint32');                          
% Desired number of samples
           header.Fs= fread(fId,1,'double');                                       
% Actual sampling frequency
           header.FsDesired = fread(fId,1,'double');                               
% Desired sampling frequency 
           header.DataStart = fread(fId,1,'uint32');                               
% Data start point in the file (byte count)
           header.TriggerStart = fread(fId,1,'uint32');                            
% Start Trigger mode (0 = Auto, 1 = Manual, 2 = External trigger)
           header.TriggerStop = fread(fId,1,'uint32');                             
% Stop Trigger mode (0 = Auto, 1 = Manual, 2 = External trigger)
           header.DaqSingleEnded = fread(fId,1,'uint32');                          
% Single ended Daq mode (1 = Single ended)
           header.DaqBipolar = fread(fId,1,'uint32');                              
% Bipolar ended Daq mode (1 = Bipolar)
           header.UseCamera = fread(fId,1,'uint32');                               
% Camera use (0 = NO, 1 = YES)
           strLength = fread(fId,1,'uint32'); 
           header.VideoFilename = char(fread(fId,strLength,'char'))';              
% Associated Video File name
           % Read in Channel info
           for i = 1 : header.Channels
               header.ChannelInfo(i).Channel = fread(fId,1,'uint32');              
% Channel number (one based) 
               strLength = fread(fId,1,'uint32'); 
               header.ChannelInfo(i).Unit = char(fread(fId,strLength,'char'))';    
% Unit
               strLength = fread(fId,1,'uint32'); 
               header.ChannelInfo(i).Label = char(fread(fId,strLength,'char'))';   
% Label
               header.ChannelInfo(i).SystemGain = fread(fId,1,'double');           
% System Gain
               header.ChannelInfo(i).ADGain = fread(fId,1,'double');               
% AD Gain
               header.ChannelInfo(i).BitResolution = fread(fId,1,'double');        
% Bit resolution
               header.ChannelInfo(i).Bias = fread(fId,1,'double');                 
% System Bias
               header.ChannelInfo(i).HP = fread(fId,1,'double');                   
% High Pass cutoff frequency
               header.ChannelInfo(i).LP = fread(fId,1,'double');                   
