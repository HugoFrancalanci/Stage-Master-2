function visualizeCycleDetection(RHLE_distance_resampled, cycle_info, position_threshold_percent, functional_labels, functional_idx, num_cycles_detected)
   % Visualisation de la détection des cycle
     if ~isempty(RHLE_distance_resampled)
        % Figure 1: Détection des cycles avec lignes verticales
        figure;
        plot(RHLE_distance_resampled, 'b');
        hold on;
        colors = {'r', 'g', 'm'};
        
        for cycle = 1:size(cycle_info, 1)
        ...
        plot([1 length(RHLE_distance_resampled)], [position_threshold_percent position_threshold_percent], 'k--');
        title(sprintf('Détection des %d cycles pour %s basée sur RHLE', ...
            num_cycles_detected, functional_labels{functional_idx}));
        xlabel('Échantillons');
        ylabel('Distance RHLE (% déplacement max)');
        legend('Position RHLE', sprintf('Seuil %d%%', position_threshold_percent));
        
        % Figure 2: Zones colorées
        figure;
        plot(RHLE_distance_resampled, 'b');
        hold on;
        ...
        title(sprintf('Position complète de RHLE pour %s avec %d cycles détectés', ...
            functional_labels{functional_idx}, num_cycles_detected));
        xlabel('Échantillons');
        ylabel('Distance RHLE (% déplacement max)');
        legend('Position RHLE', sprintf('Seuil %d%%', position_threshold_percent));
    end
